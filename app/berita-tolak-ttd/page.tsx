...
function BeritaTolakContent() {
    const searchParams = useSearchParams();
    const nopProperti = searchParams.get('nop') || '32.73.040.001.012-0043.0';
    ...
    const [tindakanLanjutan, setTindakanLanjutan] = useState<string>('...');
    ...
    return (
        <main className="w-full max-w-md mx-auto min-h-screen relative overflow-x-hidden bg-[#f8fafc] pb-5">
            ...
            <div ref={pdfRef} className="w-[93%] mx-auto border border-[#C5C5D3] bg-white mt-4 flex flex-col justify-center items-center gap-5 py-5 px-5 shadow-xs">
                ...
                {/* Alasan Penolakan */}
                <div className="w-full border-l-4 border-l-[#00236F] text-[#00236F] px-1">
                    <h3 className="font-mono font-bold text-[11px]">ALASAN PENOLAKAN & TINDAKAN LANJUTAN</h3>
                </div>
                <div className="w-full bg-[#F4F3FA] border border-[#C5C5D3] rounded-sm flex flex-col gap-3 px-4 py-4 font-public-sans text-[13px]">
                    <div>
                        <h4 className="font-semibold text-[#1A1B21]">Alasan Penolakan:</h4>
                        <p className="text-[#444651] mt-1 italic">"{alasanPenolakan}"</p>
                    </div>
                    <hr className="border-[#C5C5D3]"/>
                    <div>
                        <h4 className="font-semibold text-[#1A1B21]">Tindakan Lanjutan:</h4>
                        <p className="text-[#444651] mt-1 italic">"{tindakanLanjutan}"</p>
                    </div>
                </div>
                ...
                <div className="w-full flex justify-between items-start gap-3 text-center text-[#1A1B21]">
                    ...
                    <div className="flex flex-col items-center px-2 flex-1 min-w-0">
                        ...
                        <div className="h-[60px] flex items-start justify-center flex-col">
                            <span className="text-[10px] text-gray-400 mt-4 italic">Tolak validasi tidak memerlukan TTD penilai</span>
                        </div>
                        <hr className="border-[#444651] w-[70%] mx-auto border-t pb-1"/>
...
export default function BeritaAcaraTolakPage() {
    return (
        <Suspense fallback={
